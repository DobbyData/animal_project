...
ResultSet  rsOld = pstmtOld.executeQuery(); // 쿼리를 실행하고 결과를 ResultSet 객체에 담는다. 

String sqlRe = "select substr(happenDt, 1, 4) as Years, count(case when kind='0' then 1 end) as Dogs, count(case when kind='1' then 1 end) as Cats, count(case when kind='2' then 1 end) as The_Others, count(case when processState like '%입양%' then 1 end) as Adop from animal_total group by substr(happenDt, 1, 4);";
			
PreparedStatement pstmtRe = conn.prepareStatement(sqlRe); // prepareStatement에서 해당 sql을 미리 컴파일한다. 
ResultSet rsRe = pstmtRe.executeQuery(); // 쿼리를 실행하고 결과를 ResultSet 객체에 담는다. 
